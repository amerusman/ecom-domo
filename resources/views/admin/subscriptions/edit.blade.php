    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.subscriptions.update", [$subscription->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="email" name="email" id="email" value="{{ old('email', $subscription->email) }}" required>
                @if($errors->has('email'))
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('unsubscribed') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="unsubscribed" id="unsubscribed" value="1" {{ $subscription->unsubscribed || old('unsubscribed', 0) === 1 ? 'checked' : '' }}>
