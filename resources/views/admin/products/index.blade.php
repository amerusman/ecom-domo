            ...
            <table class=" table table-bordered table-striped table-hover datatable datatable-Product">
                <thead>
                    <tr>
                        ...
                        <th>
                            {{ trans('cruds.product.fields.showcase') }}
                        </th>
                        <th>
                            {{ trans('cruds.product.fields.on_sale') }}
                        </th>
                        <th>
                            &nbsp;
                        </th>
                ...
                <tbody>
                    @foreach($products as $key => $product)
                        <tr data-entry-id="{{ $product->id }}">
                            ...
                            <td>
                                <span style="display:none">{{ $product->in_stock ?? '' }}</span>
                                <input type="checkbox" disabled="disabled" {{ $product->in_stock ? 'checked' : '' }}>
                            </td>
                            <td>
                                {{ ($product->prices && is_array($product->prices)) ? implode(', ', $product->prices) : '' }}
                            </td>
                            <td>
                                {{ $product->brand->name ?? '' }}
                            </td>
                            <td>
                                <span style="display:none">{{ $product->showcase ?? '' }}</span>
                                <input type="checkbox" disabled="disabled" {{ $product->showcase ? 'checked' : '' }}>
                            </td>
                            <td>
                                <span style="display:none">{{ $product->on_sale ?? '' }}</span>
                                <input type="checkbox" disabled="disabled" {{ $product->on_sale ? 'checked' : '' }}>
                            </td>
                            <td>
                                @can('product_show')
